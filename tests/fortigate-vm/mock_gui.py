...
Creates the QLocalServer socket vpnProcess::setup() expects
(/tmp/openfortiGUI), accepts the VPN child process's connection and then either
triggers ACTION_STOP or closes the connection. This makes the GUI stop path in
vpnProcess::closeProcess() testable headless.

Usage: mock_gui.py <stop|disconnect> <profile-name> <log-file> [timeout]
"""
import os
import socket
import struct
