...
void tiConfVpnProfiles::saveVpnProfile(const vpnProfile &profile)
{
    ...
    QRegularExpressionMatch match = rexpName.match(profile.name);
    if(!match.hasMatch())
    {
...
void tiConfVpnProfiles::readVpnProfiles()
{
    ...
    while(it_profileDirs.hasNext())
    {
		...
		QRegularExpressionMatch match = rexpName.match(vpnprofilename);
                if(!match.hasMatch())
                {
...
{
    QRegularExpression rexpName(openfortigui_config::validatorName);
    QRegularExpressionMatch match = rexpName.match(group.name);
    if(!match.hasMatch())
    {
...
void tiConfVpnGroups::readVpnGroups()
{
    ...
    while (it_localgroupdir.hasNext())
    {
	    ...
	    QRegularExpressionMatch match = rexpName.match(vpngroupname);
            if(!match.hasMatch())
            {
